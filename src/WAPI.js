...
window.WAPI.sendMessageWithThumb = function (thumb, url, msg, title, description, chatId, done) {
    var chatSend = WAPI.getChat(chatId);
    if (chatSend === undefined) {
    ...
    chatSend.sendMessage(url+msg, { linkPreview: linkPreview, mentionedJidList: [], quotedMsg: null, quotedMsgAdminGroupJid: null });
...
window.WAPI.download = function (url, dones) {
    let xhr = new XMLHttpRequest();
    ...
    xhr.onload = function () {
        if (xhr.readyState == 4) {
            if (xhr.status == 200) {
                ...
                reader.readAsDataURL(xhr.response);
                reader.onload = function (e) {
                    dones(reader.result.substr(reader.result.indexOf(',') + 1))
                };
            } else {
                console.error(xhr.statusText);
            }
        ...
        if (!window.WAPI._newMessagesDebouncer && window.WAPI._newMessagesQueue.length > 0) {
            window.WAPI._newMessagesDebouncer = setTimeout(() => {
                ...
                removeCallbacks.forEach(function (rmCallbackObj) {
                    ...
                    window.WAPI._newMessagesCallbacks.splice(callbackIndex, 0);
                });
            }, 1000);
        }
    }
    // if(newMessage.isSentByMe){
    //     console.log(newMessage.isSentByMe)
    // }
});
...
window.WAPI.sendImage2 = function (imgBase64, chatid, filename, caption, done) {
    //var idUser = new window.Store.UserConstructor(chatid);
    var idUser = new window.Store.UserConstructor(chatid, { intentionallyUsePrivateConstructor: true });
    // create new chat
    return Store.Chat.find(idUser).then((chat) => {
        ...
        if (Debug.VERSION === '0.4.613') {
            mc.processAttachments([mediaBlob], chat, 1).then(() => {
                var media = mc.models[0];
                media.sendToChat(chat, {caption:caption, mentionedJidList: listUser});
                if (done !== undefined) done(true);
            });
        } else {
